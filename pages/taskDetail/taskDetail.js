...
Page({
  ...
  onLoad: function (options) {
    const { status, id, processDefinitionId, processInstanceId, formKey } =
      options
  ...
  fetchTaskDetail: function (id, configKey) {
    ...
    request({
      ...
      const visibleButtons = pageConfig.btnList.filter((btn) =>
        btn.isVisible(serverData),
      )
      ...
      this.setData({
        ...
        handleBeforeClose: rejectBtn
          ? this.handleBeforeClose.bind(this, rejectBtn.actionPayload.url)
          : null,
      })
  ...
  fetchWorkflow: function (instanceId) {
    request({
      ...
      const timeline = res.data.map((item) => ({
        text: item.name,
        desc: `${item.updated}\n${
          item.taskAuditRemarks ? `备注：${item.taskAuditRemarks}\n` : ''
        }${item.executorName}`,
      }))
  ...
  handleReapply(payload) {
    if (payload.type === 'KEY_PROJECT_CO_OPERATION') {
      wx.navigateTo({
        url:
          '/pages/reapplyProjectCooperation/reapplyProjectCooperation?initialData=' +
          encodeURIComponent(JSON.stringify(this.__initialData)),
      })
      return
    }
    wx.navigateTo({
      url: `/pages/taskEdit/taskEdit?type=${
        payload.type
      }&initialData=${encodeURIComponent(JSON.stringify(this.__initialData))}`,
    })
  },
